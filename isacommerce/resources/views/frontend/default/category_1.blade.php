                                ...
                                <ul class="tree-menu">
                                    @foreach($data['category'] as $c)
                                        @if($c->parent_id == $data['title_1']->id)
                                            <li><span></span><a href="{{ url('c/'.$data['title_1']->category_seo.'/'.$c->category_seo)}}">{{ $c->category_name }}</a>
                                            <ul class="tree-menu">
                                                @foreach($data['category'] as $ca)
                                                    @if($ca->parent_id == $c->id)
                                                        <li><span></span><a href="{{ url('c/'.$data['title_1']->category_seo.'/'.$c->category_seo.'/'.$ca->category_seo)}}">{{ $ca->category_name }}</a>
                                                    @endif
                                                    </li>
                                                @endforeach
                                            </ul>
                    ...
                    <div class="block_content">
                        ...
                        <div class="layered layered-filter-price">
                            ...
                            <div class="layered-content slider-range">
                                <form method="post" action="{{ url('c/filter_1') }}" name="priceForm_1">
                                {{ csrf_field() }}
                                <input type="hidden" name="seo" value="{{$data['title_1']->category_seo}}"/> 
                            ...
                            <div class="layered-content slider-range">
                                ...
                                <form method="post" action="{{ url('c/filter_1') }}" name="priceForm_2">
                                {{ csrf_field() }}
                                <input type="hidden" name="seo" value="{{ $data['title_1']->category_seo }}"/> 
                                    <ul class="check-box-list">
                                        <li>
            ...
            <div class="center_column col-xs-12 col-sm-9" id="center_column">
                ...
                <div id="view-product-list" class="view-product-list">
                    ...
                    <ul class="row product-list grid" id="product-list-index">
                        ...
                        <li class="col-sx-12 col-sm-4 product-list-index">
                                ...
                                <div class="right-block">
                                    ...
                                    <div class="content_price">
                                        ...
                                        if(!empty($p->discount_amount ) && ($p->is_active == 'Y') && ($p->expiry > $date )){
                                            ...
                                            echo "<br/>";
                                            echo '<span class="price old-price"><br/></span>';
                                        }
                                        ?>
                                    </div>
                ...
                <input type="hidden" id="seo" value="{{$data['seo']}}">
                <!-- ./view-product-list-->
                <div class="sortPagiBar">
                    <div class="bottom-pagination" id="div-pagination">
                        <nav id="pagination">
                          {{ $data['product']->links() }}
                        </nav>
                        <!--  <ul class="pagination" id="ul-pagination">
                            ...
                            <li><a class="pagination-list" href="#">4</a></li>
                            <li><a class="pagination-list" href="#">5</a></li>
                            <li>
                    ...
                    <div class="sort-product">
                        <select name = "sort-list">
                            ...
                            <option value="purchase_count|desc">Terlaris</option>
                        </select>
...
<script type="text/javascript" src="{{ url('theme/frontend/default/assets/js/filter-category-1.js')}}"></script>
<script type="text/javascript" src="{{ url('theme/frontend/default/assets/js/accounting.min.js')}}"></script>
<script type="text/javascript" src="{{ url('theme/frontend/default/assets/js/category.js')}}"></script>
<script type="text/javascript">
     $(document).ready(function() {
        $('select[name="sort-list"]').on('change', function() {
            var sort_type = $(this).val();
            var seo = $('#seo').val();
            $.ajax({
                url: 'http://localhost/isacommerce/category-1/'+seo+'/'+sort_type,
                type: "GET",
                dataType: "json",
                success:function(product) {
                    var j = 0;
                    var k = 0;
                    $("ul#product-list-index").empty();
                    $.each(product, function(i, key){
                        if(key.old_price == null){
                            var old_price = '<br/>';
                        }else{
                    ...
                    for(k=2; k<5; k++){
                        li += '<li><a class="pagination-list" href="#">'+k+'</a></li>';
                    }*/
                    $("nav#pagination").empty();
                    $("div#div-pagination").append('<ul class="pagination" id="ul-pagination"><li><a href="#" aria-label="Previous"><span aria-hidden="true">«</span></a></li><li><a class="pagination-list" href="#">1</a></li><li><a class="pagination-list" href="#">2</a></li><li><a class="pagination-list" href="#">3</a></li><li><a class="pagination-list" href="#">4</a></li><li><a class="pagination-list" href="#">5</a></li><li><a href="#" aria-label="Next"><span aria-hidden="true">»</span></a></li></ul>');
                }
            });
        });
    });
...
<!-- <script type="text/javascript">
    $(document).ready(function() {
        $('#ul-pagination').on('click', '.pagination-list', function() {
            alert('oi');
        });
    });
</script> -->
