            ...
            <div class="page-title">
                ...
                <div class="page-breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="active">{{ $parent_product->product_name }}</li>
                        ...
                        <div class="panel panel-white">
                            ...
                            <div class="panel-body">
                                @if(Auth::guard('admin')->user()->can('product-add'))
                                    <a href="{{ url('/isa-cms/products/combination/create') }}" class="btn btn-success btn-addon m-b-sm"><i class="fa fa-plus"></i> Add New Data</a>
                                @endif
                                    ...
                                    <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                       <thead>
                                        ...
                                        <tbody>
                                            ...
                                            <?php 
                                            $enable = ($p->status == "Y" ? "Yes" : "No");
                                            if(!empty($p->discount->is_active)){
                                                $discount_enable = ($p->discount->is_active == "Y" ? "Yes" : "No");
                                            }else{
                                                $discount_enable = '-';
                                            }
                                            $amount    = (!empty($p->discount->discount_amount) ? number_format($p->discount->discount_amount) : "-");
                                            $operation = (!empty($p->discount->discount_operation) ? '('.$p->discount->discount_operation.')' : "");
                                            $expiry    = (!empty($p->discount->expiry) ? date('F d, Y', strtotime($p->discount->expiry)) : "-");
                                            ?>
                                                <tr>
                                                    <td>{{ $no }}.</td>
                                                    <td>
                                                        @foreach($value_pair as $v)
                                                            @if($v->product_id == $p->id)
                                                                {{ $v->attribute_name }} : {{ $v->value_name }} <br/>
                                                            @endif
                                                        @endforeach
                                                    </td>
                                                    <td>{{ number_format($p->price) }}</td>
                                                    <td>{{ $amount }} {{ $operation }}</td>
                                                    <td>{{ $discount_enable }}</td>
                                                    <td>{{ $expiry }}</td>
                                                    <td>{{ $p->weight }}</td>
                                                    <td>{{ $p->quantity }}</td>
                                                    <td>{{ $enable }}</td>
                                                    <td>
                                                        <div aria-label="Justified button group" role="group" class="btn-group btn-group-justified">
                                                            @if(Auth::guard('admin')->user()->can('product-edit'))
                                                                <a role="button" class="btn btn-warning" href="{{ url('/isa-cms/products/combination/edit',[$p->id]) }}" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                                            @endif
                                                            @if(Auth::guard('admin')->user()->can('product-delete'))
                                                                <a role="button" class="btn btn-danger" href="#" data-toggle="modal" data-target="#modal-delete-confirmation" data-action-target="{{ url('/isa-cms/products/combination/delete',[$p->id]) }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                                            @endif
                                                        </div>
                                                    </td>
                                                </tr>
