            ...
            <div class="page-title">
                ...
                <div class="page-breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li><a href="{{ url('/isa-cms/attributes') }}">Attribute</a></li>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="panel panel-white">
                            <div class="panel-heading clearfix">
                                <h4 class="panel-title">Value List</h4>
                            </div>
                            <div class="panel-body">
                                @if(Auth::guard('admin')->user()->can('product-add'))
                                    <a href="{{ url('/isa-cms/attributes/values/create') }}" class="btn btn-success btn-addon m-b-sm"><i class="fa fa-plus"></i> Add New Data</a>
                                @endif
                                <div class="table-responsive">
                                    <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                        ...
                                        <tbody>
                                            <?php $no=1; ?>
                                            @foreach($value as $v)
                                                <?php $last_update = ($v->updated_at != null ? $v->updated_at : '-'); ?>
                                                <tr>
                                                    <td>{{ $no }}</td>
                                                    <td>{{ $v->value }}</td>
                                                    <td>{{ $v->created_at }}</td>
                                                    ...
                                                    <td>
                                                        <div aria-label="Justified button group" role="group" class="btn-group btn-group-justified">
                                                            @if(Auth::guard('admin')->user()->can('product-edit'))
                                                                <a role="button" class="btn btn-warning" href="{{ url('/isa-cms/attributes/values/edit',[$v->id]) }}" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                                            @endif
                                                            @if(Auth::guard('admin')->user()->can('product-delete'))
                                                                <a role="button" class="btn btn-danger" href="#" data-toggle="modal" title="Delete" data-target="#modal-delete-confirmation" data-action-target="{{ url('/isa-cms/attributes/values/delete',[$v->id]) }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                                            @endif
                                                        </div>
                                                    </td>
                                                </tr>
