                        ...
                        <div class="panel panel-white">
                            ...
                            <div class="panel-body">
                                @if(Auth::guard('admin')->user()->can('product-add'))
                                    <a href="{{ url('/isa-cms/attributes/create') }}" class="btn btn-success btn-addon m-b-sm"><i class="fa fa-plus"></i> Add New Data</a>
                                @endif
                                <div class="table-responsive">
                                    <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                        ...
                                        <tbody>
                                            ...
                                            <?php $enable = ($a->is_active == "Y" ? "Yes" : "No");  ?>
                                                <tr>
                                                    <td>{{ $no }}</td>
                                                    <td>{{ $a->name }}</td>
                                                    <td>{{ $a->description }}</td>
                                                    ...
                                                    <td>
                                                        <div aria-label="Justified button group" role="group" class="btn-group btn-group-justified">
                                                            @if(Auth::guard('admin')->user()->can('product-edit'))
                                                                <a role="button" class="btn btn-info" href="{{ url('/isa-cms/attributes/values/list',[$a->id]) }}" data-toggle="tooltip" data-placement="top" title="Value"><i class="fa fa-tag" aria-hidden="true"></i></a>
                                                                <a role="button" class="btn btn-warning" href="{{ url('/isa-cms/attributes/edit',[$a->id]) }}" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                                            @endif
                                                            @if(Auth::guard('admin')->user()->can('product-delete'))
                                                                <a role="button" class="btn btn-danger" href="#" data-toggle="modal" title="Delete" data-target="#modal-delete-confirmation" data-action-target="{{ url('/isa-cms/attributes/delete',[$a->id]) }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                                            @endif
                                                        </div>
                                                    </td>
                                                </tr>
